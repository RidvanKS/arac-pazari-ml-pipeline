...
def render_car_svg(parca_durumlari):
    """Parça durumlarına göre renklenen + TIKLANABİLİR araba SVG'si."""
    def renk(parca):
        d = parca_durumlari.get(parca, "belirtilmemis")
        return DURUM_RENK.get(d, "#6b7280")

    def durum_label(parca):
        d = parca_durumlari.get(parca, "belirtilmemis")
        return DURUM_LABELS.get(d, d).split(" (")[0]  # Emoji + isim

    # Her path'i <a href> ile saralım — tıklayınca query param tetiklenir
    def clickable_path(parca, path_attrs):
        """Bir SVG path'i tıklanabilir yap."""
        label = PARCA_LABELS.get(parca, parca)
        durum = durum_label(parca)
        return f'''
        <a href="?clicked_part={parca}" style="cursor:pointer;">
          <title>{label} — Şu an: {durum} (tıkla değiştir)</title>
          {path_attrs}
        </a>
        '''

    svg = f"""
    <div style="background:#0f172a;padding:1rem;border-radius:15px;border:1px solid #2d3348;">
    <div style="text-align:center;color:#94a3b8;font-size:0.9rem;margin-bottom:0.5rem;">
        👆 <b>Parçalara tıkla</b> — durum sırayla değişir: 🟢→🟡→🟠→🔴
    </div>
    <svg version="1.1" style="height:380px;width:100%;display:block;margin:auto;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 430">
      <defs>
        ...
        <style>
          a:hover path {{ opacity: 0.7; stroke-width: 1.5; stroke: #60a5fa; }}
          path {{ transition: all 0.2s ease; }}
        </style>
      </defs>
      <g fill="none" fill-rule="evenodd">
        <g transform="translate(156.5 219.5) rotate(-90) translate(-188.5 -144.5)">
          {clickable_path('sol_on_kapi', f'<path d="m106.51 55.944c-0.52167 0.93363-0.66394 2.0147-0.33197 2.9974 0.85364 2.506 2.5609 4.5207 4.7899 5.7492l4.6476 2.506c9.5798 5.2087 20.345 7.9113 31.158 7.9113h13.421l3.13-17.248c1.3279-7.2233 1.9918-14.643 1.9918-21.965v-25.847c-2.4187 0-8.0622-0.049138-13.706-0.049138-4.3156 0-7.7776 0-10.196 0.049138-1.8496 0-3.794 1.081-5.3115 2.9483-1.1856 1.425-2.2764 2.9483-3.2723 4.5207-1.0433 1.6707-1.6599 2.9483-2.229 4.1767-0.71137 1.4741-1.3753 2.8992-2.6084 4.619-1.2805 1.769-2.798 3.4397-4.5053 4.9138-3.4146 2.9483-6.3075 6.388-8.5838 10.319l-0.047424 0.049138v0.049138l-8.3467 14.299zm5.027-0.88449c2.0393-1.769 4.6476-2.7517 7.3508-2.7517h40.548c0.80622 0 1.4227 0.73707 1.3279 1.5724l-2.4187 16.412c-0.23712 1.5724-1.5176 2.7026-3.0352 2.6535l-10.149-0.19655c-9.4849-0.19655-18.733-2.85-26.937-7.7638l-3.13-1.8673c-1.8021-1.081-3.2723-2.6535-4.2208-4.5699-0.61652-1.1793-0.33197-2.6535 0.66394-3.4888z" fill="{renk("sol_on_kapi")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sol_arka_kapi', f'<path d="m166.04 58.376l-3.0398 16.732 2.9448-0.14764c18.476-0.98425 36.62-6.7421 52.483-16.634 3.4197-2.1653 5.9845-5.561 7.0769-9.5472 3.2297-11.467 3.9897-23.72 2.1848-35.531l-0.28498-1.7224c-0.14249-0.88582-0.85493-1.5256-1.7574-1.5256h-57.613v25.886c0 7.5295-0.66495 15.108-1.9948 22.49zm-0.28498 12.352l3.8472-15.994c0.42747-1.7224 1.8524-2.9527 3.5622-3.0512l34.055-1.9193v-0.049212c0-2.559 1.9948-4.6752 4.5121-4.6752h7.3144c0.61745 0 1.1874 0.14764 1.7574 0.34449z" fill="{renk("sol_arka_kapi")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('on_tampon', f'<path d="m340 201.35c3.7652 1.6125 7.7313 2.4968 11.798 2.4968h15.864c3.2632 0 5.9742-2.8609 6.1248-6.5021 0.70284-16.958 1.1547-34.643 1.1547-53.005v-0.41614c0-18.518-0.40162-36.36-1.1547-53.422-0.15061-3.6412-2.8616-6.5021-6.1248-6.5021h-15.864c-4.0664 0-8.0325 0.83227-11.798 2.4968v114.85z" fill="{renk("on_tampon")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sol_on_camurluk', f'<path d="m59.277 54.594s6.1176 1.9503 15.529 0.97517l5.3646 0.29255s9.2234 7.5088 12.047 7.2163c2.8235-0.34131 7.7175-8.4352 7.7175-8.4352l10.635-19.503c-0.14118 0.24379-15.2 3.4131-22.164-1.414-6.0234-4.1932-10.682-10.824-11.482-17.846l-0.79999-4.8759s-9.3175-0.39007-12.329 6.5824c-3.0117 6.9725-6.5411 9.4592-6.5411 9.4592s-0.94116 10.629 0.79999 13.262c1.6941 2.5842 1.2235 14.286 1.2235 14.286z" fill="{renk("sol_on_camurluk")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sag_on_camurluk', f'<path transform="translate(267.88 27.237) scale(-1) rotate(180) translate(-267.88 -27.237)" d="m234.26 49.983l53.188-9.0296s15.014-4.4657 16.577-8.6861c1.563-4.2204 2.3681-7.0176 1.563-10.109-0.80516-3.0917-2.8418-10.502-2.8418-10.502s3.3154-6.1833-0.61572-6.1833c-3.9311 0-15.958-0.98148-15.958-0.98148s2.3211 32.474-25.531 32.907c-25.568 0.39668-24.904-28.637-24.904-28.637h-5.8815s5.7309 23.212 0 41.222h4.4042z" fill="{renk("sag_on_camurluk")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sag_arka_camurluk', f'<path transform="translate(267.88 261.26) scale(-1 1) rotate(180) translate(-267.88 -261.26)" d="m234.26 284.01l53.188-9.0296s15.014-4.4657 16.577-8.6861c1.563-4.2204 2.3681-7.0176 1.563-10.109-0.80516-3.0917-2.8418-10.502-2.8418-10.502s3.3154-6.1833-0.61572-6.1833c-3.9311 0-15.958-0.98148-15.958-0.98148s2.3211 32.474-25.531 32.907c-25.568 0.39668-24.904-28.637-24.904-28.637h-5.8815s5.7309 23.212 0 41.222h4.4042z" fill="{renk("sag_arka_camurluk")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sag_on_kapi', f'<path d="m114.9 247.46l0.047425 0.098276c2.2764 3.9311 5.1693 7.4199 8.5838 10.319 1.7547 1.4741 3.2723 3.1448 4.5053 4.9138 1.233 1.7198 1.897 3.1448 2.6084 4.619 0.61652 1.2285 1.233 2.5552 2.229 4.1767 0.99592 1.5724 2.0867 3.0957 3.2723 4.5207 1.5176 1.8181 3.462 2.8992 5.3115 2.9483 2.4661 0.049138 5.8806 0.049138 10.196 0.049138 5.5961 0 11.287-0.049138 13.706-0.049138v-25.847c0-7.3707-0.66394-14.741-1.9918-21.965l-3.13-17.248h-13.469c-10.813 0-21.578 2.7517-31.158 7.9113l-4.6476 2.506c-2.2764 1.2285-3.9362 3.2431-4.7899 5.7492-0.33197 0.98276-0.1897 2.0638 0.33197 2.9974l8.3941 14.299z" fill="{renk("sag_on_kapi")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sag_arka_kapi', f'<path d="m169.03 253.22v25.886h57.66c0.85493 0 1.6149-0.63976 1.7574-1.5256l0.28498-1.7224c1.8049-11.811 1.0449-24.065-2.1848-35.531-1.1399-3.9862-3.6572-7.3819-7.0769-9.5472-15.911-9.9409-34.055-15.65-52.531-16.634l-2.9448-0.14764 3.0398 16.732c1.3299 7.3819 1.9948 14.961 1.9948 22.49z" fill="{renk("sag_arka_kapi")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('sol_arka_camurluk', f'<path d="m58.43 234.06s6.5264-2.0807 16.567-1.0403l5.7232-0.3121s9.8398-8.0106 12.852-7.6985c3.0122 0.36412 8.2333 8.999 8.2333 8.999l11.346 20.807c-0.15061-0.26008-16.216-3.6412-23.646 1.5085-6.426 4.4735-11.396 11.548-12.25 19.038l-0.85345 5.2017s-9.9402 0.41614-13.153-7.0223c-3.213-7.4385-6.9782-10.091-6.9782-10.091s-1.0041-11.34 0.85345-14.149c1.8073-2.7569 1.3053-15.241 1.3053-15.241z" fill="{renk("sol_arka_camurluk")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('kaput', f'<path d="m230 100s14.961 40.833 0 87.129h53.968s20.633-8.1667 18.876-43.07c-1.7571-34.904-18.876-44.059-18.876-44.059h-53.968z" fill="{renk("kaput")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('bagaj_kapagi', f'<path d="m95.64 100.03h-23.897s-10.743-1.3004-10.743 13.004v65.594s1.7069 8.7909 8.4843 8.7909h26.156s-8.5345-37.712 0-87.389z" fill="{renk("bagaj_kapagi")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('tavan', f'<path d="m126.16 111s-10.794 28.349-1.1547 64.501h63.658s8.7855-32.771 0-64.501h-62.503z" fill="{renk("tavan")}" stroke="#374151" stroke-width="0.5"/>')}

          {clickable_path('arka_tampon', f'<path d="m36.941 86.497c-3.7652-1.6125-7.7313-2.4968-11.798-2.4968h-15.864c-3.2632 0-5.9742 2.8609-6.1248 6.5021-0.70284 16.958-1.1547 34.643-1.1547 53.005v0.41614c0 18.518 0.40162 36.36 1.1547 53.422 0.15061 3.6412 2.8616 6.5021 6.1248 6.5021h15.864c4.0664 0 8.0325-0.83228 11.798-2.4968v-114.85z" fill="{renk("arka_tampon")}" stroke="#374151" stroke-width="0.5"/>')}
        </g>
      </g>
    </svg>
...
# ════════════════════════════════════════════════════════════════
qp = st.query_params
if "clicked_part" in qp:
    clicked = qp["clicked_part"]
    if clicked in PARCA_LABELS:
        # Durum sırası: orijinal → lokal_boyali → boyali → degismis → orijinal
        cycle = ["orijinal", "lokal_boyali", "boyali", "degismis"]
        current = st.session_state.parca_durumlari.get(clicked, "orijinal")
        try:
            idx = cycle.index(current)
            next_state = cycle[(idx + 1) % len(cycle)]
        except ValueError:
            next_state = "lokal_boyali"
        st.session_state.parca_durumlari[clicked] = next_state
    # Query param'ı temizle (URL kirli kalmasın)
...
with col_result:
    ...
    # SVG göster
    st.markdown(render_car_svg(st.session_state.parca_durumlari), unsafe_allow_html=True)
